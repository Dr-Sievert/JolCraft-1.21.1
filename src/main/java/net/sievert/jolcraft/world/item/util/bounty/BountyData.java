...
public record BountyData(BountyObjective objective) {
                        ...
                        (buf, obj) -> {
                            if (obj instanceof EntityObjective(Holder<EntityType<?>> entity, int amount)) {
                                ...
                                ResourceLocation id = entity.unwrapKey()
                                        .orElseThrow(() -> new IllegalStateException("Unkeyed entity holder in BountyObjective"))
                                        .location();
                            ...
                            ResourceLocation id = io.item().unwrapKey()
                                    .orElseThrow(() -> new IllegalStateException("Unkeyed item holder in BountyObjective"))
                                    .location();

                            buf.writeResourceLocation(id);
                            buf.writeVarInt(io.amount());
                        },
                        buf -> {
                            ...
                            if (amount < 1) {
                                throw new IllegalStateException("Invalid bounty objective amount from network: " + amount);
                            }
                            ...
                            return switch (kind) {
                                case ITEM -> {
                                    ...
                                    if (value == null) {
                                        throw new IllegalStateException("Unknown item id in BountyObjective: " + id);
                                    }
                                    if (value == Items.AIR) {
                                        throw new IllegalStateException("BountyObjective item must not be air");
                                    }
                                    yield new ItemObjective(value.builtInRegistryHolder(), amount);
                                }
                                case ENTITY -> {
                                    Registry<EntityType<?>> entities = buf.registryAccess().lookupOrThrow(Registries.ENTITY_TYPE);
                                    EntityType<?> value = entities.getValue(id);
                                    if (value == null) {
                                        throw new IllegalStateException("Unknown entity id in BountyObjective: " + id);
                                    }
                                    yield new EntityObjective(value.builtInRegistryHolder(), amount);
                                }
                            };
                        }
                );
    ...
    // =====================================================================
    // BountyData Codec + StreamCodec: { "objective": { ... } }
    // =====================================================================
