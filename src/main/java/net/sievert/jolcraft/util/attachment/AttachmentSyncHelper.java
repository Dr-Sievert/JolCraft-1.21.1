...
public class AttachmentSyncHelper {
    ...
    public static void syncAll(ServerPlayer player) {
        // Dwarvish language
        boolean knowsLang = net.sievert.jolcraft.util.attachment.DwarvenLanguageHelper.knowsDwarvishBypassCreative(player);
        JolCraftNetworking.sendToClient(player, new ClientboundLanguagePacket(knowsLang));

        // Ancient Dwarvish language
        boolean knowsAncient = net.sievert.jolcraft.util.attachment.AncientDwarvenLanguageHelper.knowsAncientDwarvishBypassCreative(player);
        JolCraftNetworking.sendToClient(player, new ClientboundAncientLanguagePacket(knowsAncient));

        // Reputation tier
        int tier = net.sievert.jolcraft.util.attachment.DwarvenReputationHelper.getTierBypassCreative(player);
        JolCraftNetworking.sendToClient(player, new ClientboundReputationPacket(tier));

        // Endorsements
        Set<ResourceLocation> endorsements = net.sievert.jolcraft.util.attachment.DwarvenReputationHelper.getAllEndorsementsBypassCreative(player);
        JolCraftNetworking.sendToClient(player, new ClientboundEndorsementsPacket(endorsements));

        // Tome Unlocks
        Set<String> unlocks = net.sievert.jolcraft.util.attachment.TomeUnlockHelper.getAllUnlocks(player);
