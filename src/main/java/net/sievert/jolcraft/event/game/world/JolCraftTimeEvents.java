...
public final class JolCraftTimeEvents {
    ...
    public static void onSleepFinished(SleepFinishedTimeEvent event) {
        ...
        FermentingCauldronBlockEntity.handleSleepFinished(level, event.getNewTime());

        for (ServerPlayer player : level.players()) {
            if (HearthAttachmentHelper.lastLitDay(player) == -1L) continue;
            HearthAttachmentHelper.clearLastLitDay(player);
        }
    }
}